...
run_body "GET /parsed/change-program/issues (제목 검색)" \
  "curl -H '$H_API' '$BASE_URL/parsed/change-program/issues?q=수정&limit=5'"

run_body "GET /parsed/change-program/issues/{issueKey}/svn-commits" \
  "curl -H '$H_API' '$BASE_URL/parsed/change-program/issues/$CP_ISSUE_KEY/svn-commits'"

run_body "POST /parsed/change-program/svn-diff-revision" \
