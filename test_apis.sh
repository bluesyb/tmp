...
run_csv() {
  ...
  echo ""
  echo "▶ $label"
  code=$(eval "$@" -s -o "$outfile" -w "%{http_code}")
  if [[ "$code" =~ ^2 ]]; then
    echo "  ✔ HTTP $code → $outfile 저장됨"
    head -5 "$outfile" 2>/dev/null
    PASS=$((PASS+1))
  else
    echo "  ✘ HTTP $code"
    cat "$outfile" 2>/dev/null | head -5
    rm -f "$outfile"
    FAIL=$((FAIL+1))
  fi
}
...
# run_body "GET /parsed/project/{projectKey}/versions (전체 - 느림)" \
...
# run_body "GET /parsed/project/{projectKey}/version/{versionName}/issues (전체 - 느림)" \
...
run_csv "GET /parsed/project/{projectKey}/issues/change-program/csv" "change-program-$VERSION_NAME.csv" \
...
run_csv "POST /parsed/change-program/source-changes/csv" "source-changes.csv" \
  "curl -X POST -H '$H_API' -H '$H_JSON' '$BASE_URL/parsed/change-program/source-changes/csv' \
    -d '{\"issue_keys\":[\"$ISSUE_KEY_1\",\"$ISSUE_KEY_2\"]}'"

run_body "GET /parsed/change-program/issues (전체 검색)" \
  "curl -H '$H_API' '$BASE_URL/parsed/change-program/issues?offset=0&limit=5'"

run_body "GET /parsed/change-program/issues (이슈키 검색)" \
  "curl -H '$H_API' '$BASE_URL/parsed/change-program/issues?issueKey=$CP_ISSUE_KEY&limit=5'"

run_body "GET /parsed/change-program/issues (제목 검색)" \
  "curl -G -H '$H_API' --data-urlencode 'summary=ORDAP-PO' '$BASE_URL/parsed/change-program/issues?limit=5'"
...
run_body "POST /parsed/svn/diff" \
  "curl -X POST -H '$H_API' -H '$H_JSON' '$BASE_URL/parsed/svn/diff' \
    -d '{\"svn_tag_url_old\":\"$SVN_TAG_URL_OLD\",\"svn_tag_url_new\":\"$SVN_TAG_URL_NEW\"}'"
